...
import { UsernameFetcherStatic } from "@/components/username-fetcher-static"
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const checkApiAvailability = async () => {
      ...
      try {
        // Use a relative URL to avoid CORS issues on Vercel
    ...
    <main className="flex min-h-screen flex-col items-center justify-center p-4 bg-gradient-to-b from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
      ...
      {isLoading ? (
        ...
        <div className="w-full max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Username Fetcher on the left */}
          <div>{isStatic ? <UsernameFetcherStatic /> : <UsernameFetcher />}</div>

          {/* Email Fetcher on the right */}
          <div>{isStatic ? <EmailFetcherStatic /> : <EmailFetcher />}</div>
        </div>
      )}
